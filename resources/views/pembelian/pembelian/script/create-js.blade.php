@push('custom-js')
    ...
    <script>
        ...
        $('#supplier').find('option').map(function() {
            list_supplier.push(`<option value="${$(this).val()}">${$(this).html()}</option>`)
        }).get()
        ...
        $('select[name="kode_po"]').change(function() {
        ...
        function get_kode() {
            $.ajax({
                url: "/beli/pembelian/generate-kode/" + $('input[name="tanggal"]').val(),
                type: 'GET',
                success: function(data) {
